/**
 * Stop hook: 读取 Claude Code JSONL → 输出与 /export 一致的终端风格 .txt
 * 输出: ${CLAUDE_EXPORT_DIR:-<cwd>/chat}/YYYYMMDD--<session_id>.txt（每会话一份，每轮覆盖）
 */
...
async function main() {
  ...
  const text = formatExportTranscript(rows);
  if (!text.trim()) process.exit(0);

  const exportDir =
    process.env.CLAUDE_EXPORT_DIR || path.join(cwd, "chat");
...
async function resolveExportPath({ exportDir, stdin, transcriptPath, rows }) {
  ...
  return path.join(exportDir, `${ymd}--${safeId}.txt`);
}
